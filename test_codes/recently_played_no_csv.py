...
try:
    ...
    # Végigmegyünk minden dalon és kigyűjtjük a fontos adatokat
    ...
    df['datetime'] = df['played_at'].dt.strftime('%Y-%m-%d %H:%M')
    ...
    display_columns = ['name', 'artist', 'datetime', 'duration_mins', 'popularity']
    ...
    for i, row in latest_songs.iterrows():
        print(f"   • {row['name']} - {row['artist']} ({row['datetime']})")
